...
if ($result) {
	...
	$container = $result['entity']->getContainerEntity();
	...
	if ($count) {
		...
		foreach ($uncategorized as $e) {
			$e->setCategory($result['entity']->guid);
		}
	}

	$view = "<li id=\"elgg-object-{$result['entity']->guid}\" class=\"elgg-item\">";
	elgg_set_viewtype('default');
	$view .= elgg_view_entity($result['entity']);
	$view .= '</li>';

	print json_encode(array('guid' => $result['entity']->guid, 'view' => $view));
